...
class OnboardingWindow(gtk.Window):
	def __init__(self, run_main_loop=True):
		...
		try:
			import preferences
			gtk4compat.apply_text_direction(preferences.read_language())
		except Exception:
			pass
	...
	def execute_slide7(self):
		...
		if status != 0:
			...
			gtk4compat.alert(self.window, _("Could not save certainty preference, but setup is otherwise complete."))
