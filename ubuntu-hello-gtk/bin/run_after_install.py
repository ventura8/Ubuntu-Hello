...
LOG_PATH = "/run/ubuntu-hello/postinstall.log"
LOCK_PATH = "/run/ubuntu-hello/postinstall.lock"


def _ensure_run_dir(path: Optional[str] = None) -> None:
    """Create *path* as a 0700 euid-owned directory; refuse symlinks."""
    path = RUN_DIR if path is None else path
    try:
        os.mkdir(path, 0o700)
    ...
    if not stat.S_ISDIR(st.st_mode):
        raise OSError(f"{path}: not a directory")
    if st.st_uid != os.geteuid():
        raise OSError(f"{path}: unexpected owner uid {st.st_uid}")
    os.chmod(path, 0o700)


def _prepare_privileged_path(path: str) -> None:
...
def _open_secure(path: str, flags: int, mode: int = 0o600) -> int:
    ...
    by another uid, replace it after ``lstat`` confirms it is not a symlink,
    then create a fresh euid-owned file with ``O_NOFOLLOW | O_EXCL``.
    """
    nofollow = getattr(os, "O_NOFOLLOW", 0)
    if not nofollow:
        raise OSError(f"{path}: O_NOFOLLOW is required")

    _prepare_privileged_path(path)
...
def _log(message: str) -> None:
    ...
    line = message.rstrip() + "\n"
    try:
        fd = _open_secure(LOG_PATH, os.O_WRONLY | os.O_CREAT | os.O_APPEND, 0o600)
        try:
...
def _acquire_single_flight_lock() -> Optional[int]:
    ...
    try:
        fd = _open_secure(LOCK_PATH, os.O_RDWR | os.O_CREAT, 0o600)
...
def resolve_install_user() -> Optional[str]:
    ...
    # Prefer a seated graphical user session over manager/greeter leftovers.
    # systemd 256+ columns: SESSION UID USER SEAT LEADER CLASS TTY …
    seated: Optional[str] = None
    any_user: Optional[str] = None
    try:
        ...
        for line in output.splitlines():
            ...
            if len(parts) < 3:
                continue
            user = parts[2]
            if user in ("USER", "root", "-"):
                continue
            seat = parts[3] if len(parts) > 3 else "-"
            cls = parts[5] if len(parts) > 5 else ""
            if any_user is None:
                any_user = user
            if seat not in ("", "-") and (cls in ("", "user") or "user" in cls):
                seated = user
                break
        if seated:
            return seated
...
def models_enrolled(models_dir: str = "/etc/ubuntu-hello/models") -> bool:
    """Return True if any face model files are present under *models_dir*."""
    try:
...
def launch_setup_wizard(
    user: str, env: dict[str, str], *, force_onboarding: bool = False
) -> subprocess.Popen:
    ...
    cmd.append(gtk_bin)
    # --force-onboarding only when explicitly requested; normal postinst launches
    # only when no models exist, and window.py opens the wizard in that case.
    if force_onboarding:
        cmd.append("--force-onboarding")

    log_fd = _open_secure(LOG_PATH, os.O_WRONLY | os.O_CREAT | os.O_APPEND, 0o600)
...
def main() -> int:
    ...
    if os.environ.get("DESTDIR"):
        print("post-install: DESTDIR set; skip setup wizard launch", flush=True)
        return 0
    ...
    force = os.environ.get("UH_FORCE_POSTINSTALL_GUI") == "1"
    if not force and models_enrolled():
        _log("post-install: face models already enrolled; skip setup wizard")
        return 0
    ...
    try:
        proc = launch_setup_wizard(user, env, force_onboarding=force)
    except OSError as exc:
