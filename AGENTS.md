...
| `ubuntu-hello/src/notify.py` | Desktop notification per face-auth attempt (`org.freedesktop.Notifications` via absolute `/usr/bin/gdbus`, sent on the **target user's** session bus by dropping root to that uid). One card, **updated in place** (`replaces_id`): "Looking for your face…" → recognized / not recognized / too dark, with certainty-vs-threshold, frames, fps, dark-skipped, elapsed; `[notifications] details = true` appends pid/euid/device/resolution/threshold/timeout/load. Best-effort: no session bus (greeter), missing gdbus, or a gdbus error just disables it for that run. **GNOME Shell 50 rules (read from the shell's own JS, verified live)**: the body is ONE line — the shell does `body.replace(/\n/g, ' ')` and the collapsed banner ellipsizes, so essentials first, diagnostics last; only `<b>/<i>/<u>` render. The in-progress card is `urgency=critical` because the shell never auto-hides (and auto-expands) critical banners — that is what keeps "Looking for your face…" up for the whole scan; the result update lowers urgency to normal/low which re-arms the 4 s auto-hide. `urgency=low` never banners at all. Never send `desktop-entry` (A/B: cards filed under the app never showed) — so per-app `force-expanded` is unavailable. No `transient` (failure cards must stay in the list). The id is persisted per user: privileged PAM compare writes `/run/ubuntu-hello/notify/<uid>/` (root-owned `0700`, `O_NOFOLLOW`, refuse non-regular files, no path-based `chown`) so a user cannot plant a symlink for root to follow; unprivileged dry runs keep `/run/user/<uid>/ubuntu-hello-notify.id`. A detached `sleep; gdbus CloseNotification` helper (own session, as the user) removes the success card after `[notifications] success_linger` (3 s). The start() watchdog that closes a critical progress card whose helper died without a result `lstat`s the `.done` marker as root (the 0700 tree is not user-traversable) then drops uid before `gdbus`. The PAM module forwards the PAM service as `compare.py` argv[2] so titles read `Face recognized · sudo` / `authorization` / `screen unlock`; the app icon is `logo.png` cropped to its drawn bounds. Config `[notifications] enabled/details/success_linger`. Tests: `tests/test_notify.py`. |
