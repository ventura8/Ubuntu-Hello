...
| `scripts/uh-apt-deps.sh` | Shared apt build/runtime deps for `install.sh` / `uninstall.sh` (GTK/Babel, multi-DE theme tools, wallet PAM, polkit). Records newly added packages under `/var/lib/ubuntu-hello/apt-packages-added.list` (path is not overridable via `UH_APT_MARKER` / `sudo -E`); clean removal allows auto transitive deps; untracked manual packages refused; auto checks avoid `apt-mark \| grep -q` under pipefail and use `</dev/null` inside `while read` loops. |
| `scripts/package-configure.sh` | Shared post-install configure (SHA256 model verify, permissions, config restore, PAM, polkit); installs pinned `dlib==19.24.9` via pip when missing on the live host (sets `CMAKE_POLICY_VERSION_MINIMUM=3.5` for modern CMake; marker for prerm). Sourced by `install.sh` and by deb/rpm/snap/AppImage/Flatpak hooks. |
...
* **Pinned CI deps** (explicit version tags/numbers only — never commit SHAs, never the `latest` alias):
  ...
  * GHA actions: explicit tags (e.g. `actions/checkout@v7.0.1`, `docker/setup-buildx-action@v4.3.0`, `softprops/action-gh-release@v3.0.2`)
