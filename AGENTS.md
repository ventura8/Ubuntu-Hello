...
* **SonarQube Cloud (mandatory for Python changes)**: static analysis runs locally via `./scripts/ci-sonar.sh`, and in CI as a **step inside the `coverage` job** of `check.yml` for pushes and same-repository pull requests (fork PRs are skipped — no secret). There is no separate `sonar` job. Both read the same `sonar-project.properties` (repo root — the scanner's mandated base dir).
  * **Local**: `SONAR_TOKEN=… ./scripts/ci-sonar.sh` (Docker `sonarsource/sonar-scanner-cli:12.2.0.4256_8.1.0`, version-pinned, never `:latest`). The token may instead live in `.sonar-token` at the repo root (**gitignored — never commit it**). `UH_SONAR_COVERAGE=1` refreshes coverage first.
