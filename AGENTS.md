...
* **SonarQube Cloud (mandatory for Python changes)**: static analysis runs on every push/PR via the `sonar` job in `check.yml` and locally via `./scripts/ci-sonar.sh`. Both read the same `sonar-project.properties` (repo root — the scanner's mandated base dir).
