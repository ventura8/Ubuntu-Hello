...
sequenceDiagram
    ...
    activate C++
        ...
        C++->>Python: Spawn compare.py in new process group (posix_spawn, minimal env)
        activate Python
...
* **Auth notifications**: `compare.py` creates an `AuthNotifier` (`notify.py`) after reading the config, calls `start()` once the camera is open (async thread, never blocks the scan) and updates the same card on success / timeout (`exit 11`) / too dark (`exit 13`) / SIGTERM (`cancelled`, from `_signal_exit` before `os._exit(12)`). All calls go through `_notify()` which swallows every exception — a notification failure must never change an auth result. Polkit ≥ 126 sandboxing note: the helper drop-in (see `install.sh` / `package-configure.sh` / `install_config.py`) must also allow `char-tpm`, `/dev/tpm0` and `ReadWritePaths=/etc/ubuntu-hello/tpm-keys`, or TPM-sealed keyring unlock fails only under polkit (`DevicePolicy=strict` + `ProtectSystem=strict`) while working at GDM. It must also set **`ProtectHome=tmpfs` + `BindReadOnlyPaths=-/run/user`** — `ProtectHome=yes` hides `/run/user`, i.e. the user's session bus, so no desktop notification can be delivered from a polkit prompt (Settings app, App Center, `pkexec`); `tmpfs` keeps `/home`/`/root` empty inside the unit and only the runtime dir is bound back (never `read-only`/`no`: that exposed every home tree to the root helper — security review) — and `ReadWritePaths=-/run/ubuntu-hello` for the root-only notification-id state; `ubuntu-hello/tmpfiles.d/ubuntu-hello.conf` creates `/run/ubuntu-hello{,/notify}` at boot.
...
* **Session-idle self-abort (best-effort, does NOT cover GNOME/GDM's Esc)**: `compare.py` runs a daemon thread (`_watch_session_idle`) that polls its own login1 session's `IdleHint` via `busctl` (`Manager.GetSessionByPID` → `Session.IdleHint`) roughly twice a second. `_session_idle_hint()` only accepts exactly `b true`/`b false` from `busctl`; anything else (malformed output, an unexpected D-Bus type) is treated as undetermined, never as `true` — a false positive would abort a legitimate scan. The watcher is **edge-triggered**: it requires an observed `false` reading before treating a later `true` as a cancel signal, because waking a lock screen commonly starts a new auth attempt *while* `IdleHint` is still `true` from before (cleared only on successful unlock) — aborting on that initial reading would reject the normal wake-and-unlock case. On a genuine `false → true` transition it sends the process `SIGTERM` (`os.kill(os.getpid(), SIGTERM)`) rather than calling `cleanup()` directly from this thread — `cleanup()` touches `video_capture`/`gtk_proc`, which the main thread may be using concurrently, so cleanup must run through the existing main-thread `_signal_exit` handler (Python always delivers registered signal handlers on the main thread) instead of racing it. This is `compare.py` deciding to bail out on its own — it does **not** touch PAM-level password watching or `ask_pass`, so it is orthogonal to the hard rule below and does not need `workaround` set. Fails open: `_session_idle_hint()` returning `None` a few times in a row stops the watcher with no effect on the scan. Config: `core.abort_on_session_idle` (default `true`). Tests: `tests/test_compare_cleanup.py`.
  > [!NOTE]
  > **Live-verified limitation (GNOME/GDM, 2026-08-24):** captured the full D-Bus traffic (both system and session buses) across a real Esc-at-lock-screen repro. `logind`'s `IdleHint` never transitioned; neither did `org.freedesktop.portal.Inhibit`'s `screensaver-active`, nor `org.gnome.ScreenSaver.ActiveChanged` (the session stays "active"/locked the whole time — Esc only toggles GNOME Shell's internal password-prompt-vs-clock sub-view, which exposes no external signal found so far). `org.gnome.ScreenSaver.WakeUpScreen` / `Mutter.IdleMonitor` watches *do* fire on Esc, but also on any ordinary interaction (typing, mouse move), so using them risks aborting a legitimate in-progress scan — not used for that reason. Net effect: on GNOME/GDM this watcher is currently a **dormant no-op** for the Esc case specifically; it remains enabled because it is harmless (fails open) and may still help on desktop environments where `IdleHint` genuinely reflects UI state. Do not claim this "fixes" Esc-at-GNOME-lock without new evidence.
...
>
> Esc→lock-shield camera leftovers are **not** an excuse to break greeter login via the PAM-side mechanisms above. Keep `workaround=off` sequential on greeters; use process-group SIGTERM + `PR_SET_PDEATHSIG`, or `compare.py`'s own session-idle self-abort above, for cleanup — never a PAM-level password-watch widening.
...
* **Optional Settings override**: Settings → **Language** tab combo (Automatic always first + English + Whisper codes). Combo labels use **Babel/CLDR** names in the active UI language (`python3-babel`), with each language’s **native name in parentheses** when it differs (e.g. `German (Deutsch)`). Persists to `~/.config/ubuntu-hello/preferences.ini` (`[ui] language=auto|<code>`). Python `i18n` (CLI / GTK / compare) reads this before UI load; **PAM stays on the system/session locale** and does not read the preference file. Language changes apply **instantly** (reinstall gettext + rebuild the Builder UI in-process; no restart). CLI/compare pick up the preference on next process start. No `--lang` and no `config.ini` language key.
...
* **Native multi-DE Settings**: `ubuntu-hello-gtk` is **GTK 4 + GtkBuilder `.ui`** (`main.ui`, `onboarding.ui`; no Glade, no libadwaita) with stock widgets, `gtk4compat.py` for the removed GTK 3 APIs (`Gtk.main`, `Dialog.run`, `get_children`, `connect_signals`) and `theme_detect.py` on **GNOME, KDE/Plasma, XFCE, Cinnamon, MATE, Budgie, LXQt** (Ubuntu 26.04). Runtime dep is `gir1.2-gtk-4.0` / `gtk4` everywhere (deb, RPM, Arch, snap, AppImage/Flatpak host install). Validate edited `.ui` with `gtk4-builder-tool validate`; radio choices are `GtkCheckButton` + `group`. **No GTK 4.10-deprecated widgets** (`tests/test_no_deprecated_gtk.py` fails the build): use `Gtk.DropDown` via `gtk4compat.dropdown()`, `Gtk.ColumnView` via `gtk4compat.ColumnList`, `gtk4compat.alert()` (AlertDialog) / `gtk4compat.PromptWindow` instead of MessageDialog/Dialog, a `Gtk.Revealer` banner instead of InfoBar. **Never destroy and recreate the Settings toplevel** (language switch transplants content into the existing `Gtk.Window`; recreating it crashed mutter 50 and logged the user out) and never `set_visible(True)` on a `Gtk.Popover`/`Gtk.Native` subtree (maps a grabbing popup). Verify Wayland-specific behaviour on a headless compositor: `mutter --headless --wayland --no-x11 --wayland-display=<name> --virtual-monitor 1280x900` then run the E2E with `WAYLAND_DISPLAY=<name> GDK_BACKEND=wayland`. Page idiom for every wizard slide / Settings tab: symbolic glyph → title (`.uh-page-title`, scale 1.4 bold) → dimmed hint (`.uh-page-hint`, ≤60 chars wide) → content, groups in `.uh-card`; no hard-coded fonts or colours in CSS/UI. Do not rewrite as web/Electron and do not reintroduce GTK 3 APIs. CI compat = package/build **and** Settings E2E (`UH_REAL_GTK=1` + xvfb) in every `UH_CI_DE` cell; also smoke-check theme/layout/search/language/polkit on those DEs.
* Domains: `ubuntu-hello` (PAM + core Python), `ubuntu-hello-gtk` (GTK Python + Builder `.ui` + desktop). Catalogs install under `$prefix/share/locale/<lang>/LC_MESSAGES/`.
...
| `ubuntu-hello/src/notify.py` | Desktop notification per face-auth attempt (`org.freedesktop.Notifications` via absolute `/usr/bin/gdbus`, sent on the **target user's** session bus by dropping root to that uid). One card, **updated in place** (`replaces_id`): "Looking for your face…" → recognized / not recognized / too dark, with certainty-vs-threshold, frames, fps, dark-skipped, elapsed; `[notifications] details = true` appends pid/euid/device/resolution/threshold/timeout/load. Best-effort: no session bus (greeter), missing gdbus, or a gdbus error just disables it for that run. **GNOME Shell 50 rules (read from the shell's own JS, verified live)**: the body is ONE line — the shell does `body.replace(/\n/g, ' ')` and the collapsed banner ellipsizes, so essentials first, diagnostics last; only `<b>/<i>/<u>` render. The in-progress card is `urgency=critical` because the shell never auto-hides (and auto-expands) critical banners — that is what keeps "Looking for your face…" up for the whole scan; the result update lowers urgency to normal/low which re-arms the 4 s auto-hide. `urgency=low` never banners at all. Never send `desktop-entry` (A/B: cards filed under the app never showed) — so per-app `force-expanded` is unavailable. No `transient` (failure cards must stay in the list). The id is persisted per user so consecutive attempts replace the same card — as root in the root-owned `/run/ubuntu-hello/notify/<uid>.id` (0700 dir verified with `lstat`, files opened `O_NOFOLLOW`, never `chown`ed: writing under the user's own `/run/user/<uid>` would let a planted symlink redirect root), for a user's own dry run in `$XDG_RUNTIME_DIR`. Privileges are dropped with `subprocess`'s `user=/group=/extra_groups=[]` (C-level), never `preexec_fn` (compare.py is multi-threaded). A detached `sleep; gdbus CloseNotification` helper (own session, as the user) removes the success card after `[notifications] success_linger` (3 s); the same helper armed by `start()` is a watchdog for a helper SIGKILLed mid-scan and is **killed** when a result card goes out. A `_finalized` flag under the lock stops a late in-progress card from overwriting a result. With rubberstamps enabled the success card is sent only after `rubberstamps.execute()` decided (exit 0), and exit 15 sends the `rejected` (liveness failed) card; the notifier is created before the model file is read so `no_model` (exit 10) is reachable. The PAM module forwards the PAM service as `compare.py` argv[2] (titles `Face recognized · sudo` / `authorization` / `screen unlock`) and the validated `LANG`/`LANGUAGE`/`LC_*` variables; `compare.py` then applies the target user's Settings language preference (`UH_TARGET_USER` → `i18n.reload_from_preferences()`; core `i18n._` resolves through the current catalog). Config `[notifications] enabled/sound/details/success_linger`. Tests: `tests/test_notify.py`. |
...
| `ubuntu-hello/src/rubberstamps/` | Post-auth liveness check plugins (nose-tracking nod check). **Failure-mode names are inherited from Howdy and mean the opposite of what they look like**: `failsafe` = the check must pass, a timeout ABORTS authentication (fail closed); `faildeadly` = a timeout lets authentication THROUGH (fail open). Settings writes `failsafe`. The nod is detected from anchored nose displacement on the dominant axis, not a frame-to-frame delta (that was frame-rate dependent, and the detector finds a face in only ~28% of IR frames, so a late nod was missed). The challenge prompt is mirrored to the notification card: PAM's empty environment means the auth overlay can never open a display, so the overlay text alone is invisible. A stamp that raises DENIES under `failsafe`. |
| `ubuntu-hello-gtk/src/` | Administrative settings panel (`window.py`; Models tab reveals `single_model_infobar` while exactly one model is enrolled), setup wizard (`onboarding.py`), and overlay window (`authsticky.py`). Instant language rebuild + fuzzy search; `preferences.py` / `languages.py` / `search_fuzzy.py`. |
...
| `ubuntu-hello-gtk/src/tab_security.py`, `[video] certainty` + `[video] confirmations` | Match strictness. `certainty` is a distance (lower = stricter); `confirmations` is how many **separate** frames must fall under it before the match is trusted. A scan takes dozens of independent looks, so accepting on the first frame under the bar decides on the minimum of many draws — one outlier frame used to be enough. Levels: Fast 3.5/2, Balanced 3.0/3, Secure 2.6/4 (default). **Do not tighten `certainty` from one measurement**: with a single enrolled model the distance to the same face drifts ~0.4 between lighting conditions, and a value fitted to one session stops recognising that same person in the next (1.8 matched zero frames after a light change; 2.0 failed 44% of simulated logins). Tighten via `confirmations` instead. The same numbers live in `ubuntu-hello-gtk/src/onboarding.py` and, for the install-time migration, `ubuntu-hello/src/install_config.py`. |
| `scripts/vm/`, `tests/vm/` | Booted-OS tier: a clean Ubuntu, Fedora or Arch cloud image under QEMU/KVM with an emulated TPM, scenarios run inside over SSH (`UH_VM=1 pytest tests/vm/`, ~20 min per distro). Covers what containers cannot: PAM as a login path (sudo and the greeter rule via `pam_probe.py`), the polkit helper sandbox and the TPM drop-in, reboot persistence (tmpfiles.d), install/reinstall/upgrade-from-previous-release/remove on a real root. Installs packages built from the tree (`UH_VM_PKG_DIR`) or the PPA's current build; CI runs a per-distro matrix on release branches and on demand. Never involves a real face. See the test-runner skill. |
| `tests/footage/` | Recorded-footage tier: `compare.py`'s real scan loop, the agreeing-frames gate and the nod stamp on frames from the real camera (`UH_REAL_DLIB=1 pytest tests/footage/`). The only automated coverage of the recognition decision; every other suite mocks dlib. Real clips and their extracted signals stay off the repository (a face, then its biometric template); CI runs the committed synthetic set. See the test-runner skill. |
| `ubuntu-hello/src/frame_text.py` | Draws text onto a video frame for the camera test and the snapshot. `cv2.putText` only has OpenCV's Hershey fonts, which are Latin-only strokes, so every translated overlay label rendered as question marks. Pure ASCII still goes through `cv2.putText` (identical pixels, no cost); anything else is drawn with a real font resolved by `fc-match` for that script, through Pillow (**`python3-pil`** is a runtime dep). Missing Pillow or font falls back to Hershey -- a label must never stop the camera test running. |
| `ubuntu-hello/src/enroll_capture.py`, `ubuntu-hello-gtk/src/enroll.py` | Guided multi-sample enrollment: `add` walks six poses (`GUIDE_STEPS`) and stores up to 13 descriptors per model, printing `@guide <pose>` / `@progress n/total`; `enroll.run_add()` streams that on a worker thread into the wizard / Settings prompts (never `subprocess.run` on the main loop) and `notify.EnrollNotifier` mirrors the prompt on a card under the camera. Descriptor index ≠ model index: always map via `enroll_capture.flatten_models()`. |
| `ubuntu-hello-gtk/src/theme_detect.py` (`ThemeWatcher`) | Live theme following when elevated: the desktop's change feed runs as the user (`gsettings monitor` / `xfconf-query -m`, KDE/LXQt `settings.ini` file monitors); `window.setup_theme()` starts one watcher whose callback is `window.apply_user_theme(user)`. Never use `Gio.Settings` for the user's theme as root (it reads root's empty dconf). |
...
| `scripts/i18n-lint.py` | Lint JSON packs + gettext `.po` (UTF-8/JSON, `msgfmt --check`, no empty/fuzzy `msgstr`, placeholder parity, fill-pack ↔ `.pot` / `_keys.json` completeness with no stale keys, ordered `packs/` ↔ keyed JSON, no English left in, no MT sentinels, literal command text / config keys / subcommand and product names kept verbatim). CI lint stage; pytest `test_all_translations_filled`. The literal list is also what `scripts/i18n_fill_data/mt_fill_all.py` protects during machine translation. |
...
* Run background processes via safe subprocess spawning APIs (`posix_spawn` with an explicit minimal environment) rather than raw `fork()`/`exec()` or vulnerable `system()` shell-escapes.
...
* **Allowed root exceptions** that are intentionally single-purpose: `VERSION` (project semver SSOT), `meson.build` / `meson.options`, `install.sh` / `uninstall.sh`, `AGENTS.md` / adapter stubs, and similar long-standing entrypoints, plus `sonar-project.properties` (SonarQube Cloud requires it at the scanner base directory).
...
* **GHA check.yml parallelism**: **4** top-level jobs expand immediately to **17** runners (lint, coverage, 8× `compat` DE matrix, 7× `packaging` format matrix) to use OSS **20-runner** concurrency; **triggers**: `push` fires for **`master` only**, `pull_request` for every PR, plus `workflow_dispatch` — a same-repo PR branch is covered by its `pull_request` run, and firing `push` too doubled every run (the two events key the concurrency group differently, so neither cancelled the other); a branch with no PR yet needs `gh workflow run check.yml --ref <branch>`. **`concurrency.cancel-in-progress`** drops stale runs when the same PR or branch is pushed again; the packaging matrix skips **fork** `pull_request` events (`head.repo.full_name == github.repository`) and runs `scripts/ci-packaging-cell.sh` (build + smoke + live E2E); parallel cells share the bind mount — deb uses `override_dh_clean`, Flatpak stages under `UH_ARTIFACTS_DIR/.flatpak-work`. Local `./scripts/ci-pipeline.sh` fail-fast lint → coverage → compat → packaging (same cell script via `ci-packaging-matrix.sh`).
...
* **SonarQube Cloud (mandatory for Python changes)**: static analysis runs locally via `./scripts/ci-sonar.sh`, and in CI as a **step inside the `coverage` job** of `check.yml` for pushes and same-repository pull requests (fork PRs are skipped — no secret). There is no separate `sonar` job. Both read the same `sonar-project.properties` (repo root — the scanner's mandated base dir).
  * **Local**: `SONAR_TOKEN=… ./scripts/ci-sonar.sh` (Docker `sonarsource/sonar-scanner-cli:12.2.0.4256_8.1.0`, version-pinned, never `:latest`). The token may instead live in `.sonar-token` at the repo root (**gitignored — never commit it**). `UH_SONAR_COVERAGE=1` refreshes coverage first. `./scripts/ci-sonar.sh --check-token` only validates the token; CI runs it just before the scan so an **expired or revoked token fails with a message naming the problem** rather than a generic scanner error.
  * **CI**: a step **inside the `coverage` job**, not a job of its own — `check.yml` allows no job dependency gates (all 17 runners start at once), and the Cobertura XML the scan uploads is written by the step just above it. That job's checkout therefore uses `fetch-depth: 0` (new-code attribution), and a `sudo chown` step between the Docker stage and the scan reclaims the workspace — the root-run Docker stage leaves root-owned `__pycache__/` and `build-ci-*/` dirs that the runner-user scan action cannot read (`AccessDeniedException`). Do not remove that step. Uses `SonarSource/sonarqube-scan-action@v8.2.2` — the action rather than `ci-sonar.sh` **only** because it derives PR-decoration parameters from the Actions context. Fork PRs are skipped (no `SONAR_TOKEN` secret).
  * **Coverage** reaches Sonar as `artifacts/coverage/coverage.xml`, written by `run_pytest_coverage` in `scripts/ci-docker.sh`. If you move that path, update `sonar-project.properties` and the CI artifact steps in the same change set.
  * **Fixing Sonar issues** follows §4.8 fix-until-green: fix the code. Do **not** add `# noqa`, `NOSONAR`, in-code suppressions, or SonarQube "won't fix"/"false positive" resolutions to clear a finding — `scripts/no-suppressions-lint.py` rejects the in-code forms outright.
