...
7. **AuthNotifier markers**: privileged `compare.py` (PAM/root) writes notification id / `.done` files under `/run/ubuntu-hello/notify/<uid>/` with `O_NOFOLLOW` (refuse non-regular files). Do **not** restore `open(..., "w")` + path `chown` under `/run/user/<uid>/` as root — a user-planted symlink is followed and truncates an arbitrary root-writable path. Tests: `tests/test_notify.py`.
...
# pytest tests/test_compare_cleanup.py tests/test_keyring_crypto.py tests/test_notify.py -v
```
...
| `tests/test_notify.py` | AuthNotifier markers: `O_NOFOLLOW`, `/run/ubuntu-hello/notify/<uid>/` |
