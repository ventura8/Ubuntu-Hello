...
- **Notifications drop privileges**: the per-attempt desktop notification (`notify.py`) is sent on the *target user's* session bus by `setuid`/`setgid` to that user before exec'ing the absolute `/usr/bin/gdbus`; root never connects to a user bus, and the card body is markup-escaped. Failures only disable the card, never affect the auth result. Privileged marker/id files live under `/run/ubuntu-hello/notify/<uid>/` (root-owned `0700`), opened with `O_NOFOLLOW` (and `O_EXCL` for the pid tmp); path-based `chown` is not used. Root does **not** fall back to `/run/user/<uid>/` if the privileged dir cannot be created.
- **Polkit helper sandbox**: polkit ≥ 126 runs `polkit-agent-helper-1` under `DevicePolicy=strict` + `ProtectSystem=strict`; the shipped drop-in re-allows exactly the camera, `/dev/uinput`, the TPM devices (`char-tpm`, `/dev/tpm0`) and write access to `/etc/ubuntu-hello/tpm-keys` (transient `.ctx` files) — nothing broader.
...
| **AuthNotifier `.done` / id symlink follow** | Privileged compare writes notification id/done markers under `/run/ubuntu-hello/notify/<uid>/` (`0700`), `O_NOFOLLOW` (+ `O_EXCL` tmp), `fstat` regular-file check, `fchown` only on an already-open fd; never `open(..., "w")` or path `chown` under `/run/user/<uid>/` as root | Verified in `tests/test_notify.py` |
