...
- **Settings (`ubuntu-hello-gtk`) environment allowlist**: polkit pins only the interpreter and `argv[1]` (the script), so any extra `--env-KEY=value` argument reaching the elevated process is caller-controlled. `window.py` restores only the fixed display/locale set in `SESSION_ENV_ALLOWLIST` (the same set `elevate()` forwards) and drops everything else (`PATH`, `LD_*`, `PYTHON*`, `HOME`, …) with a stderr notice. As root it additionally pins `PATH` to `ROOT_SAFE_PATH` before any helper (`ubuntu-hello`, `sudo`, `gsettings`, `loginctl`) is spawned, so no inherited or injected search path can redirect a privileged exec.
...
- **Access Control Lists (ACLs)**: Credentials stored under `/etc/ubuntu-hello/` are strictly isolated:
  - `/etc/ubuntu-hello/` itself is installed `0755` (root-owned): it holds non-secret configuration (`config.ini`, `0644`) and the downloaded dlib model files. Both the source installer and the packages set it this way; a booted-OS test (`tests/vm/`) checks these modes on a clean install.
  - Everything secret lives in root-only subdirectories: `models/` (face templates), `keyring-keys/` and `tpm-keys/` are `0700`, key files `0600`. The one exception is the software-fallback master key below, which is written at the top level; its writer tightens the whole directory to `0700` when it creates the key (hosts without a TPM), which the root-only PAM and helper paths do not mind.
