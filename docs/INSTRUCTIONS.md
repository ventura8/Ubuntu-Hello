...
- Lint catalogs: `python3 scripts/i18n-lint.py` (CI lint stage; JSON UTF-8/parse, `msgfmt --check`, no empty/fuzzy `msgstr`, placeholder parity, fill-pack ↔ `.pot` completeness without stale keys, ordered `packs/` ↔ keyed JSON, no English left in, literal command text kept verbatim). Pytest: `tests/test_i18n_lint.py::test_all_translations_filled`. A literal the lint reports as translated away is restored by hand in the keyed JSON and its ordered pack.
...
```bash
sudo ubuntu-hello add        # guided: look straight, slightly left/right, chin up/down (~1 min, up to 13 samples)
