...
class TestLanguageSwitchNavigation:
	def test_switch_to_german_then_navigate_everything(self, real_i18n, gtk_pump):
		...
		try:
			...
			# And back to Automatic: English again — and LTR (manual test 2026-09-16:
			# Arabic → Automatic kept Arabic text with an LTR layout)
			_switch(win, preferences.AUTO, gtk_pump)
			assert Gtk.Widget.get_default_direction() == Gtk.TextDirection.LTR
			assert real_i18n.effective_language() == "en"
			assert real_i18n._("Models") == "Models"
			assert win.builder.get_object("modelstab").get_text() == "Models"
		finally:
			win.window.destroy()
			gtk_pump()
...
class TestArabicRightToLeft:
	def _assert_translated(self, root, catalog, where):
		...
		for label in _labels(root):
			...
			if text not in catalog:
				continue  # not a msgid: already translated text, or a string without a catalog entry
			# The rendered text IS an English msgid. Either the catalog carries the English source as
			# its "translation" (machine-translation fallback; manual test 2026-09-16: Models banner)
			# or a real translation exists and the label simply was not translated.
			if catalog[text] == text:
				assert len(text.split()) < 4, f"{where}: English fallback in catalog for {text!r}"
			else:
				raise AssertionError(f"{where}: untranslated label {text!r}")
	...
	def test_settings_mirrors_and_translates(self, real_i18n, gtk_pump, monkeypatch):
		...
		try:
			...
			win.notebook.set_current_page(_page_index(win, "abouttab"))
			gtk_pump(30)
...
class TestAutomaticAfterSavedArabic:
	def test_app_started_with_saved_arabic_returns_to_english_ltr(self, gtk_pump, tmp_path, monkeypatch, isolated_fs):
		"""The launcher imports i18n while the saved preference is already Arabic, so its
		LANG/LANGUAGE are Arabic when it elevates. The elevated app must still know the
		desktop locale (English) and switching to Automatic must give English + LTR."""
		localedir = tmp_path / "locale"
		mo_dir = localedir / "ar" / "LC_MESSAGES"
		mo_dir.mkdir(parents=True)
		subprocess.run(["msgfmt", "-o", str(mo_dir / f"{DOMAIN}.mo"), str(GTK_PO / "ar.po")], check=True)
		preferences.write_language("ar")                       # saved before the app starts
		monkeypatch.setenv("LANGUAGE", "en")                   # the desktop
		...
		for name in ("window", "onboarding", "tab_video", "tab_keyring", "tab_models", "tab_notifications"):
			...
			if mod is not None and hasattr(mod, "_"):
				monkeypatch.setattr(mod, "_", module._)
		# elevate() would forward the desktop locale, not the Arabic override
		forwarded = {}
		...
		try:
			...
			try:
				assert Gtk.Widget.get_default_direction() == Gtk.TextDirection.RTL
				assert win.builder.get_object("label_models_title") is None or True
				_switch(win, preferences.AUTO, gtk_pump)
				assert module._("Models") == "Models"
				assert Gtk.Widget.get_default_direction() == Gtk.TextDirection.LTR
				assert all("نماذج" not in lbl.get_text() for lbl in _labels(win.window))
			finally:
				win.window.destroy()
				gtk_pump()
		finally:
