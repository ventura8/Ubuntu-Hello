...
def _page_index_by_tab(notebook, builder, tab_id):
	"""Page numbers shift when a tab is added; look the page up by its tab label."""
	label = builder.get_object(tab_id)
	assert label is not None, tab_id
	for index in range(notebook.get_n_pages()):
		if notebook.get_tab_label(notebook.get_nth_page(index)) is label:
			return index
...
class TestSettingsWindowSmoke:
	...
	def test_notebook_tabs_present_and_switchable(self, settings_ui):
		...
		assert notebook.get_n_pages() == 7   # Models, Video, Notifications, Security, Keyring, Language, About
		labels = []
	...
	def test_fuzzy_search_switches_to_best_tab(self, settings_ui):
		...
		keyring = _page_index_by_tab(notebook, settings_ui["builder"], "keyringtab")
		assert notebook.get_current_page() == keyring
