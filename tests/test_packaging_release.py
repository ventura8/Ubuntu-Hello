...
def test_meson_installs_every_core_python_module():
    """Every module under ubuntu-hello/src must be installed (enroll_capture.py is imported by cli/add.py)."""
    import re
    from pathlib import Path
    root = Path(__file__).resolve().parents[1] / "ubuntu-hello" / "src"
    meson = (root / "meson.build").read_text(encoding="utf-8")
    listed = {m.split("/")[-1] for m in re.findall(r"'([A-Za-z0-9_/]+\.py)'", meson)}
