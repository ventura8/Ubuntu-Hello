...
def test_dropdown_wraps_once_and_updates_callback(fake_gtk):
	...
	# a widget that refuses attributes still gets an adapter
	class Rigid(FakeDropDown):
		__slots__ = ("selected", "handlers", "model", "search", "expression")
	rigid = Rigid()
	assert isinstance(real_dropdown(rigid), gtk4compat.IdDropDown)
