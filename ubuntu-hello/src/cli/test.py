...
if config.get("video", "recording_plugin", fallback="opencv") != "opencv":
	print(_("Ubuntu Hello has been configured to use a recorder which doesn't support the test command yet, aborting"))
	sys.exit(12)
...
cv2.namedWindow("Ubuntu Hello Test")
cv2.setMouseCallback("Ubuntu Hello Test", mouse)
...
try:
	while True:
		...
		cv2.imshow("Ubuntu Hello Test", frame)
