...
def _private_dir(path):
	...
	try:
		...
		return False
	import stat
	# Effective uid: under sudo's PAM stack the helper has ruid=user / euid=0, and the
	# root branch of state_paths() is chosen by geteuid() too.
	return stat.S_ISDIR(st.st_mode) and st.st_uid == os.geteuid() and (st.st_mode & 0o077) == 0
...
class AuthNotifier:
	...
	def __init__(self, user, service="", enabled=True, details=False, sound=True, success_linger=3.0, call_timeout=2.0):
		...
		# Serialises the gdbus calls (progress card before result card) without
	...
	def _notify(self, summary, body, urgency, expire_ms, hints=None, icon=APP_ICON, final=True):
		...
		# State transition under the state lock only (never held during I/O):
		# a result card marks the attempt final so a late progress card is
		# dropped even while another send is still on the wire.
		with self._lock:
			if final:
				...
				return False
		# Sends are serialised on their own lock so the progress card (sent from
		# start()'s thread) always reaches the server before the result card, and
		# the in-place update (replaces_id) sees the id the previous send returned.
		with self._io_lock:
			with self._lock:
				if not final and self._finalized:
					return False  # a result went out while we waited: keep it
				if self.disabled_reason:
					return False
	...
	def liveness(self, prompt, hint=""):
		"""Show a liveness challenge ("Nod to confirm") on the in-progress card.

		The auth overlay cannot ask: PAM starts compare.py with an explicit minimal
		environment (PATH only, see the module header in compare.py), so the GTK
		window has no display to open on and its prompt is never seen. The
		notification card reaches the user's session bus instead, which is the same
		path the progress and result cards already take.
		"""
		if self.disabled_reason or not prompt:
			return False
		body = self._line(
			"🙂 " + html.escape(prompt),
		...
		# Not final: the recognised / not-recognised card still has to replace this.
		return self._notify(self.title(_("Face authentication")), body, URGENCY_CRITICAL,
		                    EXPIRE_PROGRESS, {"value": 66}, final=False)
...
class EnrollNotifier(AuthNotifier):
	"""One card, updated in place, that mirrors the guided-enrollment prompts.

	`ubuntu-hello add` walks the user through small head movements. The
	prompt is shown in the wizard / Settings, but the notification banner sits
	right under the camera, where the user is looking, so the same text is
	sent there too (critical urgency keeps it on screen; the progress hint
	shows how many samples are in). Everything degrades silently like the
	auth card: no session bus, no gdbus or disabled in config = no card.
	"""

	def __init__(self, user, enabled=True, call_timeout=2.0):
		super().__init__(user, service="", enabled=enabled, details=False, sound=False,
		                 success_linger=4.0, call_timeout=call_timeout)

	def title(self, base):
		return base

	def guide(self, prompt, count=0, total=0):
		"""Show the pose to take now; *count/total* fills the progress hint."""
		if self.disabled_reason or not prompt:
			return False
		percent = int(round(100.0 * count / total)) if total else 0
		body = self._line(
			"🙂 " + html.escape(prompt),
			(html.escape(_("{count} of {total}").format(count=count, total=total)) if total else ""),
		)
		return self._notify(_("Face enrollment"), body, URGENCY_CRITICAL, EXPIRE_PROGRESS,
		                    {"value": percent}, final=False)

	def saved(self, label, samples):
		body = self._line(
			"✓ " + self._b(_("Face model saved")),
			self._b("“%s”" % label),
			html.escape(ngettext("{n} sample", "{n} samples", samples).format(n=samples)),
		)
		if self._notify(_("Face enrollment"), body, URGENCY_NORMAL, EXPIRE_SUCCESS, {"value": 100}):
			self._close_later(self.success_linger)

	def failed(self, reason):
		body = self._line("✗ " + html.escape(reason or _("Could not record a face model")))
		self._notify(_("Face enrollment"), body, URGENCY_NORMAL, EXPIRE_FAILURE)
